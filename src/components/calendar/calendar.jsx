...
const Calendar = forwardRef((props, ref) => {
    ...
    const [selectedAppointment, setSelectedAppointment] = useState(null);
    const [reloadKey, setReloadKey] = useState(0);
    const [showLogin, setShowLogin] = useState(false); // Login-Fenster steuern
    const [pendingBooking, setPendingBooking] = useState(null); // Temporärer Zustand für Termin
...
const handleToggleAppointment = useCallback(async (day, time, therapistIndex, existingAppointment) => {
    if (!userId) {
        ...
        setShowLogin(true); // Login-Fenster öffnen
        return;
    }
